...
import { LinkCreatedCard } from "@/components/links/link-created-card";
  ...
  if (createdLink) {
    return (
      <Card className="max-w-2xl">
        <CardHeader>
          <CardTitle>Link criado com sucesso!</CardTitle>
        </CardHeader>
        <CardContent>
          <LinkCreatedCard
            url={buildPublicUrl(publicDomain ?? "seu-dominio.com.br", createdLink.slug)}
            editHref={`/admin/links/${createdLink.id}/edit`}
          />
        </CardContent>
      </Card>
    );
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    {affiliatePlatforms.length === 0 ? (
                      <FormDescription>
                        Nenhuma plataforma ativa cadastrada. Crie uma em Afiliados antes de usar esta
                        opção.
                      </FormDescription>
                    ) : (
